...
DEFAULT_QUERY_GEN_PATH = PIPELINE_ROOT / "query_gen.jsonl"


# ─── Load query_gen ───────────────────────────────────────────────────────────

def load_query_gen(path: Path) -> dict[int, str]:
    """
    query_gen.jsonl: mỗi dòng là judged record có thêm field question_gen.
    Returns: {index: question_gen_text}
    """
    result = {}
    if not path.exists():
        log.info(f"query_gen: {path.name} không tồn tại → bỏ qua augmentation")
        return result
    with open(path, encoding="utf-8") as f:
        for i, line in enumerate(f, 1):
            ...
            try:
                ...
                log.warning(f"query_gen dòng {i}: JSON lỗi — {e}")
                continue
            idx = rec.get("index")
            q   = rec.get("question_gen", "").strip()
            if idx is not None and q:
                result[int(idx)] = q
    log.info(f"query_gen: {len(result)} records loaded từ {path.name}")
    return result
...
def build_master(data: dict, out_path: str | Path = DEFAULT_MASTER_PATH) -> list[dict]:
    ...
    # query_gen        = load_query_gen(DEFAULT_QUERY_GEN_PATH)  # {index: gen_query}
    query_gen = {}
    ...
    for idx in sorted(eligible_indices):
        ...
        master.append(base_record)

        # ── Augmented record từ question_gen ──────────────────────────────────
        gen_query = query_gen.get(idx, "").strip()
        if gen_query and gen_query != query_text:
            master.append({
                **base_record,
                "query":        gen_query,
                "is_augmented": True,
...
def _print_summary(master: list[dict], neg_stats: Counter, out_path: Path):
    ...
    n_original  = sum(1 for r in master if not r.get("is_augmented"))
    n_augmented = sum(1 for r in master if r.get("is_augmented"))
    ...
    print(f"    original        : {n_original:>5}")
    print(f"    augmented       : {n_augmented:>5}  (từ question_gen)")
