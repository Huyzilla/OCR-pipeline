...
def process_folder(
    ...
    print(f"Bỏ qua {skipped_files} file đã xử lý")
    print(f"Cần chạy {len(pdf_files)} file\n")
    ...
    print(f"Tổng thời gian: {int(batch_elapsed//60):3d}m {int(batch_elapsed%60):02d}s")
    
    if times:
        print(f"Avg/file:       {sum(times)/len(times):6.1f}s")
        print(f"Min/Max:        {min(times):6.1f}s / {max(times):6.1f}s")
