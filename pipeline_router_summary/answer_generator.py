...
class AnswerGeneratorQwen:
    ...
    TRA_CUU_PROMPT = """Dựa vào thông tin sau đây, vui lòng trả lời câu hỏi.
...
ANSWER:"""
    ...
    TINH_TOAN_COT_PROMPT = """Dựa vào context sau, phân tích câu hỏi từng bước.
...
Phân tích từng bước (KHÔNG cần kết luận đáp án):"""
    ...
    TINH_TOAN_ANSWER_PROMPT = """Dựa vào phân tích sau, chọn đáp án đúng.
    ...
    def generate_answer(
        ...
        if intent == "tinh_toan":
            reasoning = self._generate(
                self.TINH_TOAN_COT_PROMPT.format(
                    context=context, question=question, options_text=options_text
                ),
                max_new_tokens=400,
            )
            response = self._generate(
                self.TINH_TOAN_ANSWER_PROMPT.format(
