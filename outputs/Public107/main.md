...
- Nếu đạo hàm của hàm số tại xt: f′(xt)>0 thì xt nằm về bên phải so với x∗ (và ngược lại). Để điểm tiếp theo xt+1 gần với x∗ hơn, chúng ta cần di chuyển xt về phía bên trái, tức về phía *âm*. Nói các khác, **chúng ta cần di chuyển ngược dấu với đạo hàm**:xt+1=xt+ΔTrong đó Δ là một đại lượng ngược dấu với đạo hàm f′(xt).
...
**from** \_\_future\_\_ **import** division, print\_function, unicode\_literals
...
Từ hình minh họa trên ta thấy rằng ở hình bên trái, tương ứng với x0=−5, nghiệm hội tụ nhanh hơn, vì điểm ban đầu x0 gần với nghiệm x∗≈−1 hơn. Hơn nữa, với x0=5 ở hình bên phải, *đường đi* của nghiệm có chứa một khu vực có đạo hàm khá nhỏ gần điểm có hoành độ bằng 2. Điều này khiến cho thuật toán *la cà* ở đây khá lâu. Khi vượt qua được điểm này thì mọi việc diễn ra rất tốt đẹp.

Learning rate khác nhau

Tốc độ hội tụ của GD không những phụ thuộc vào điểm khởi tạo ban đầu mà còn phụ thuộc vào *learning rate*. Dưới đây là một ví dụ với cùng điểm khởi tạo x0=−5 nhưng learning rate khác nhau:
...
Việc tính toán đạo hàm của các hàm nhiều biến là một kỹ năng cần thiết. Một vài đạo hàm đơn giản có thể được [tìm thấy ở](https://machinelearningcoban.com/math/#bang-cac-dao-ham-co-ban) đây.
...
```
w lr = np.dot(np.linalg.pinv(A), b)
...
2025-09-28 17.44.53\_AIRace

Solution found by formula:  $w = [[4.00305242 \ 2.99862665]]$ 

2025-09-28 17.44.53\_AIR
...
Chúng ta cùng quay lại một chút với Giải tích I năm thứ nhất đại học: [Khai triển](http://mathworld.wolfram.com/TaylorSeries.html)  [Taylor.](http://mathworld.wolfram.com/TaylorSeries.html)

Với ε rất nhỏ, ta có hai xấp xỉ sau:

$$f(x+\epsilon)\approx f(x)+f'(x)\epsilon+f''(x)2\epsilon 2+\dots$$

$$v\grave{a}:f(x-\epsilon)\approx f(x)-f'(x)\epsilon+f''(x)2\epsilon 2-\dots$$

$$T\grave{u} d\acute{o} ta c\acute{o}:f(x+\epsilon)-f(x)\epsilon\approx f'(x)+f''(x)2\epsilon+\dots=f'(x)+O(\epsilon) \quad (3)$$

$$f(x+\epsilon)-f(x-\epsilon)2\epsilon\approx f'(x)+f(3)(x)6\epsilon 2+\dots=f'(x)+O(\epsilon 2) \quad (4)$$
...
**print**( 'Checking gradient...', check\_grad(np.random.rand(2, 1), cost, grad))
...
Ví dụ về đường đồng mức trong các bản đồ tự nhiên. (Nguồn: [Địa lý 6: Đường](http://vforum.vn/diendan/showthread.php?90166-Dia-ly-6-Duong-dong-muc-la-nhung-duong-nhu-the-nao-)  [đồng mức là những đường như thế](http://vforum.vn/diendan/showthread.php?90166-Dia-ly-6-Duong-dong-muc-la-nhung-duong-nhu-the-nao-) nào?)
