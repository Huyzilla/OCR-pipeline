...
Thuật toán tối ưu bầy kiến (ACO) nghiên cứu các hệ thống nhân tạo dựa vào hành vi tìm kiếm của bầy kiến thực và được sử dụng để giải quyết các vấn đề về tối ưu rời rạc.Thuật toán bầy kiến siêu tìm kiếm(ACO meta\_heuristic) lần đầu tiên được Dorigo, Di Caro và Gambardella đề xuất vào năm 1999.
...
$$p_{ij}^{k} = \frac{\left(\tau_{ij}\right)^{\alpha} \left(\eta_{ij}\right)^{\beta}}{\sum_{l \in N_{i}^{k}} \left(\tau_{il}\right)^{\alpha} \left(\eta_{il}\right)^{\beta}}, j \in N_{i}^{k}$$
...
$$\Delta \tau_{ij}^{k} = \begin{cases} 1/C^{k}, & \text{n\'eu ki\'en k qua cung (i,j)} \\ 0, & \text{ngược lại} \end{cases}$$
 (2.6)

Với: C<sup>k</sup>là độ dài đường đi của con kiến thứ k sau khi hoàn thành đường đi, tức là bằng tổng các cung thuộc đường đi mà kiến đã đi qua.
...
- ⮚ Cho q<sup>o</sup> là một hằng số cho trước (0<q0<1)
- ⮚ Chọn ngẫu nhiên một giá trị q trong khoảng [0,1]
- ⮚ Nếu q<q<sup>0</sup> kiến k chọn điểm j di chuyển tiếp theo dựa trên giá trị lớn nhất của thông tin mùi và thông tin heuristic có trên cạnh tương ứng với công thức:

$$j = \arg_{l \in N_i^k} \max \left( \tau_{il} (\eta_{il})^{\beta} \right) \qquad (2.7)$$

⮚ Nếu q>q<sup>0</sup> kiến k sẽ chọn đỉnh j chưa được đi qua trong tập láng giềng của I theo một quy luật phân bổ xác xuất được xác định theo công thức sau:

$$p_{ij}^{k} = \frac{(\tau_{ij})^{\alpha} (\eta_{ij})^{\beta}}{\sum_{k \in N_{i}^{k}} (\tau_{ii})^{\alpha} (\eta_{ii})^{\beta}}, j \in N_{i}^{k}$$
...
$$\tau_{ij} \leftarrow (1 - \rho)\tau_{ij} + \rho\tau_0 \tag{2.9}$$

Với *p*: là tham số bay hơi năm trong khoảng (0,1)

$$\int_{0}^{\infty} = 1/(nC_{nm})$$
...
$$\Delta \tau_{ij}^{best} = \begin{cases} 1/C_{best} & \text{n\'eu ki\'en qua cạnh (i,j)} \\ 0 & \text{ngược lại} \end{cases}$$
...
Thuật toán MMAS đã thêm vào giá trị cận trên và giá trị cận dưới cho thông tin pheromone gọi là *τ*min và *τ*max

Sau mỗi lần cập nhật giá trị thông tin mùi  $\tau_{ij}$  nếu  $\tau_{ij} < \tau_{min}$  thì sẽ gán  $\tau_{ij} = \tau_{min}$  và nếu  $\tau_{ij} > \tau_{max}$  thì gán  $\tau_{ij} = \tau_{max}$ 

Giá trị cận trên  $\tau_{max}$  thường được thiết lập với công thức sau:

$$\tau_{max} = \frac{1}{\rho C_{best}}$$

Giá trị cận dưới  $\tau_{min}$  được xác định bằng công thức:
...
- $\blacktriangleright$  Đầu tiên, m con kiến được xếp hạng theo thứ tự giảm dần dựa theo chất lượng lời giải mà nó thu được. Ví dụ:  $(S_1, S_2, \dots S_{m-1}, S_m)$  trong đó  $S_1$  là phương án tốt nhất.
- Pheromone chỉ được đặt thêm trên các cung của 6 -1 con kiến có lời giải tốt nhất. Lượng pheromone cũng phụ thuộc trực tiếp vào thứ hạng sắp xếp của con kiến.
...
se như sau: 
$$\tau_{rs} \leftarrow \tau_{rs} + \sigma . \Delta \tau_{rs}^{gb} + \Delta \tau_{rs}^{rank} .$$
 
$$\Delta \tau_{rs}^{gb} = \begin{cases} f(C(S_{global-best})), & a_{rs} \in S_{global-best} \\ 0, & \text{trái lại.} \end{cases}$$

Trong đó Và

$$\Delta \tau_{rs}^{rank} = \begin{cases} \sum_{\mu=1}^{\sigma-1} (\sigma - \mu).f(C(S_{\mu}^{'})), & a_{rs} \in S_{\mu}^{'} \\ 0, & \text{trái lại.} \end{cases}$$

Tóm tắt thủ tục cập nhật pheromone của thuật toán này:

```
1 Procedure daemon actions
```

- for each  $S_k$  do local search  $(S_k)$  {optional} 2
- rank  $(S_1,...,S_m)$  in decreasing order of solution 3 quality into  $(S_1, ..., S_m)$
- if  $(\text{best}(S_1, S_{global-best}))$ 4
...
17.50.10\_AI Race 8 for each edge  $a_{rs} \in S'_{\mu}$  do

$$\sigma_{rs} = \tau_{rs} + (\sigma - \mu) \cdot f\left(C\left(S_{\mu}'\right)\right)$$

- 10 end for
- end for 11
- for each edge  $a_{rs} \in S_{global-best}$  do 12

13 
$$\tau_{rs} = \tau_{rs} + \sigma . f\left(C\left(S_{global-best}\right)\right)$$
...
Thuật toán được đưa ra bởi Cordon vào năm 1999. Thuật toán này bao gồm một thuật toán mở rộng khác của AS là MMAS (về luật di chuyển và việc bay hơi của pheromone). Bên cạnh đó trong thuật toán này còn quan tâm tới của việc tối ưu cục bộ một cách hệ thống để nâng cao chất lương lời giải của con kiến. Trong thuật toán BWAS có 3 daemon action thêm vào gồm có:

- Dâu tiên, áp dụng luật có tên best-worst pheromone update để tăng cường pheromone trên các đoạn đường đi qua bởi lời giải tốt nhất toàn cục (global best solution). Thêm vào đó luật này sẽ phạt những cạnh của lời giải tồi nhất trong lần lặp Scurrent-worst.
- Ap dụng Pheromone trail mutation để đi theo các hướng khác nhau trong quá trình tìm kiếm.
- > BWAS có cơ chế khởi tạo lại thông tin pheromone khi thuật toán bị đình trệ, bằng cách thiết lập pheromone trail cho tất cả các thành phần bằng  $\tau_0$ .
...
```
for each S_k do local search (S_k)
2
```

$$S_{current-best} = best\_solution (S_k)$$

3 
$$S_{current-best} = best\_solution (S_{current-best})$$
4 **if** (best ( $S_{current-best}$ ,  $S_{global-best}$ ))

$$S_{global-best} = S_{current-best}$$

for each edge  $a_{rs} \in S_{global-best}$  do 7

8 
$$\tau_{rs} = \tau_{rs} + p. f\left(C(S_{global-best})\right)$$
...
$$\tau_{\textit{threshold}} = \frac{\textit{sum}}{|S_{\textit{globad-best}}|}$$

$$S_{current-worst} = worst\_solution (S_k)$$

12 
$$S_{current-worst} = worst\_solution (S_k)$$
13 **for** each edge  $a_{rs} \in S_{current-worst}$  and  $a_{rs} \notin S_{global-best}$  **do**
14  $\tau_{rs} = (1-p).\tau_{rs}$ 
...
$$z = generate\_random\_value\_in\_[0,1]$$

```
if (z <= P_m)
19
                     s = generate random value in [1,..., 1]
20
                       a = generate random value in [0,1]
21
                     if (a=0) \tau_{rs} = \tau_{rs} + mut
22
                     else \tau_{rs} = \tau_{rs} - mut
23
        end if
24
       end for
...
**(Directed or undirected) ring topology**: trong mô hình directed ring colony cụm kiến (z +1 mod *p )* + 1 là hàng xóm của cụm *i* cho tất cả các cụm kiến và trong mô hình undirected ring colony cụm kiến *(i* - 1 mod *p )*  + 1 là hàng xóm của cụm kiến
