...
### **THUẬT TOÁN SẮP XẾP KINH ĐIỂN - PHẦN 1** <sup>L</sup>ần ban hành: 1
...
(i = 0; i < n-1; i++) \quad \{ \text{ //thuật toán selection sort} \\ \text{ (int i, j, min_idx; //min_idx là vị trí để arr[min_index] nhỏ nhất} \\ \text{ for (i = 0; i < n-1; i++)} \quad \{ \text{ //dwoêt n } 1 \text{ -1.} \text{ } \} 
                                        min_idx = i; //vị trí số bé nhất tạm thời là i
                                        for \ (j=i+1; \ j < n; \ j++) \{ \ /\!/tim \ v! \ tr'i \ s\acuteob\acutee \ nh \acuteat \ arr[i+1],..., \ arr[n-1]
                                                  if (arr[i] < arr[min idx])
                                                          min idx = j;
                                        swap(&arr[min idx], &arr[i]);//tráo đổi arr[min idx] và arr[i]
                                }
