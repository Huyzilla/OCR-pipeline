...
|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|--|--|
| Content-base attention | $\mathrm{score}(\boldsymbol{s}_t,\boldsymbol{h}_i) = \mathrm{cosine}[\boldsymbol{s}_t,\boldsymbol{h}_i]$                                             |  |  |
| Additive(*)            | $\operatorname{score}(\boldsymbol{s}_t, \boldsymbol{h}_i) = \mathbf{v}_a^\top \operatorname{tanh}(\mathbf{W}_a[\boldsymbol{s}_t; \boldsymbol{h}_i])$ |  |  |
| Location-              | $\alpha_{t,i} = \operatorname{softmax}(\mathbf{W}_a s_t)$                                                                                            |  |  |
| Base                   | Note: This simplifies the softmax alignment to only depend on the target position.                                                                   |  |  |
| General                | $\text{score}(\bm{s}_t, \bm{h}_i) = \bm{s}_t^{\top} \mathbf{W}_a \bm{h}_i$ where $\mathbf{W}_a$ is a trainable weight matrix in the attention layer. |  |  |
| Dot-Product            | $\operatorname{score}(\boldsymbol{s}_t, \boldsymbol{h}_i) = \boldsymbol{s}_t^\top \boldsymbol{h}_i$                                                  |  |  |
| Scaled Dot-            | $\text{score}(\pmb{s}_t, \pmb{h}_i) = \frac{\pmb{s}_t^{\scriptscriptstyle \intercal} \pmb{h}_i}{\sqrt{n}}$                                           |  |  |
