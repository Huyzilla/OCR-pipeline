# **XỬ LÝ TIẾNG VIỆT TRONG CÔNG NGH<sup>Ệ</sup> THÔNG TIN** <sup>L</sup>ần ban hành: 1
...
Việc phân tích một câu nói hay một câu trong văn bản ở các tầng từ ngữ âm (1) đến tầng ngữ pháp (3) gọi là *phân tích sơ bộ* (shallow parsing). Nếu phân tích thêm cả tầng ngữ nghĩa (từ (1) đến (4)) ta sẽ có *phân tích đầy đủ* (fully parsing). Trong các vấn đề của xử lí
...
- các *mô hình ngẫu nhiên* đóng một vai trò lớn, tiêu biểu là các mô hình Markov ẩn (HMM, Hidden Markov Model) trong xử lý tiếng nói;
