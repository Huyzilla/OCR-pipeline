...
# Xây dựng mô hình xác suất P(Y|X)
...
# Linear-Chain CRFs dang ma trận
...
$$Z_{\theta}(x) = \left( M_1(x) \times M_2(x) \times \dots \times M_{n+1} \times 1_{|\mathcal{Y}'| \times 1} \right)_{0,0}$$

Công thức xác suất có điều kiện có thể biểu diễn dưới dạng ma trận:

$$p_{\theta}(Y = y | X = x) = \frac{\prod_{i=1}^{n} M_{i}(y_{i-1}, y_{i} | x)}{\left(\left(\prod_{i=1}^{n} M_{i}(x)\right) \times 1_{|y_{i}| \times 1}\right)_{0,0}}$$

Biểu diễn này hữu ích trong việc huấn luyện và suy luận mô hình CRFs. 2025-09-28 17.43.14.
