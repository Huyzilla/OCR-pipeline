...
def get_embedding_truncate_dim(args) -> int | None:
    value = getattr(args, "embedding_truncate_dim", None)
    return int(value) if value else None


def encode_texts(texts: list[str], embedder, args, *, batch_size: int = 64, show_progress_bar: bool = True):
    return embedder.encode(
        texts,
        batch_size=batch_size,
        show_progress_bar=show_progress_bar,
        normalize_embeddings=True,
        truncate_dim=get_embedding_truncate_dim(args),
    )


def embedding_dim_ok(embedding, expected_dim: int | None) -> bool:
    if expected_dim is None:
        return True
    shape = getattr(embedding, "shape", None)
    return bool(shape) and int(shape[-1]) == expected_dim


def load_or_build_query_cache(questions: list[dict], embedder, cache_path: Path, args) -> dict[int, object]:
    cache = {}
    expected_dim = get_embedding_truncate_dim(args)
    if cache_path.exists():
        ...
        if isinstance(loaded, dict):
            cache = {
                ...
                if isinstance(k, str) and embedding_dim_ok(v, expected_dim)
            }
    ...
    if missing:
        ...
        embeddings = encode_texts(texts, embedder, args, batch_size=64, show_progress_bar=True)
...
def build_chroma_collection(args, chunk_texts: list[str], chunk_ids: list[str], embedder):
    ...
    if args.chunk_emb_cache.exists():
        ...
        if (
            ...
            and cached.shape[0] == len(chunk_texts)
    ...
    if chunk_embs is None:
        chunk_embs = encode_texts(chunk_texts, embedder, args, batch_size=64, show_progress_bar=True)
...
def prepare_retrieval(args, questions: list[dict]):
    ...
    if truncate_dim:
        print(f"  Embedding truncate dim: {truncate_dim}")
    embedder = SentenceTransformer(args.embedding_model, truncate_dim=truncate_dim)
    query_cache = load_or_build_query_cache(questions, embedder, args.query_cache, args)
